          ...
          <div style={{ padding: '24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px', flexWrap: 'wrap' }}>
              ...
              <span style={{ fontSize: '13px', color: '#9ca3af', display: 'flex', alignItems: 'center', gap: '4px' }}>
                <Clock size={12} /> {timeAgo(post.created_at)} · Posted by u/{post.author_username}
              </span>
            </div>
          ...
          {comments.length === 0 ? (
            ...
            comments.map((c: any) => (
              <div key={c.id} style={{
                ...
                <p style={{ fontSize: '15px', color: '#374151', lineHeight: 1.6, margin: 0, paddingLeft: '42px' }}>{c.content}</p>
              </div>
            ))
          )}
        </div>
